...
function News({ dispatch, loading }) {
  ...
  // const [firstImageUpload, setFirstImageUpload] = useState(false);
  // const [secondImageUpload, setSecondImageUpload] = useState(false);
  ...
  const onFinish = (val) => {
    // const year = val?.date?.slice(0, 1);
    // console.log(year, 'year');
    // const years = val?.date.split('');
    // console.log(years, 'years');
    const formData = new FormData();
    formData.append('title', val.title);
    formData.append('date', dayjs(val?.date).format());

    // how to get only year from val.date
    // formData.append('year', years[3]);
    // formData.append('year', val?.date?.slice(0, 1));
  ...
  return (
    <div className="container mx-auto">
      <Page title={recordId ? 'Edit News' : 'Add News'}>
        <div>
                  ...
                  <Col xl={12} lg={12} md={12} sm={24} xs={24}>
                    <div className="p-5 bg-white rounded-md">
                      ...
                      <Col xl={24} lg={24} md={24} sm={24} xs={24}>
                        <div className=" text-left ">Upload News</div>
...
const mapStateToProps = ({ user, loading }) => ({
  ...
  loading: loading.effects['news/addNewsList'],
});

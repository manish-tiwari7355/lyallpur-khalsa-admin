...
var configPath = path.join(__dirname, 'fabric.rc');
...
if (isTsProject) {
    try {
        if (!fs.existsSync(configPath) && fs.existsSync(path.join('configPath', '..'))) {
            isJsMoreTs(configPath).then(function (jsMoreTs) {
            ...
            var cacheTime = fs.readFileSync(configPath).toString();
            if (new Date().getDate() !== parseInt(cacheTime, 10)) {
                fs.unlink(configPath, function () { });
            }
        }
    }
    catch (error) {
